...
void Core::CoreInit()
{
	...
	_projectionMatrix = glm::perspective(glm::pi<float>() / 4.0f, 1.0f, 0.05f, 20.0f);
...
void Core::KeyCallback(GLFWwindow* w, int key, int scancode, int action, int mods)
{
	...
	case GLFW_KEY_D:
		...
		return;
	case GLFW_KEY_R:
		_callback_object->OnKeyR(down);
		return;
	case GLFW_KEY_F:
		_callback_object->OnKeyF(down);
		return;
	case GLFW_KEY_Q:
		_callback_object->OnKeyQ(down);
		return;
	case GLFW_KEY_E:
